...
import { Search, Star, Bell, ChevronDown, Sparkles, LogOut, User, Key, HelpCircle } from "lucide-react";

export default function Navbar() {
  const { searchQuery, setSearchQuery, activePage, setActivePage } = usePodcast();
  ...
  return (
    <header className="h-20 bg-background/50 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-8 sticky top-0 z-30 ml-64">
      {/* Search Input Bar */}
      <div className="w-96 relative group">
        ...
        <input
          ...
          placeholder="Search podcasts, topics, keywords..."
          className="w-full text-sm py-2.5 pl-11 pr-12 rounded-xl bg-[#13131A] border border-white/5 focus:border-primary/50 text-white placeholder-text-muted outline-none transition-all focus:ring-1 focus:ring-primary/20"
        />
        <div className="absolute right-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-[9px] font-bold text-text-muted select-none flex items-center gap-0.5">
          <span>⌘</span>
          <span>K</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-5">
        {/* Upgrade Button */}
        <button className="flex items-center gap-2 px-4.5 py-2.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-white font-semibold text-xs transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer shadow-neon-purple">
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-left hidden sm:block">
              ...
              <p className="text-[9px] text-text-muted capitalize">{session.role}</p>
            </div>
