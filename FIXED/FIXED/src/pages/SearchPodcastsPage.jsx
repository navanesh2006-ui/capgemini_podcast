...
export default function SearchPodcastsPage() {
  const { podcasts, selectPodcast, setActivePage } = usePodcast();
  ...
  const filteredPodcasts = publicPodcasts.filter((pod) => {
    ...
    const matchesSearch =
      ...
      pod.title.toLowerCase().includes(localSearchQuery.toLowerCase().trim()) ||
      pod.host.toLowerCase().includes(localSearchQuery.toLowerCase().trim()) ||
      pod.category.toLowerCase().includes(localSearchQuery.toLowerCase().trim()) ||
      pod.description.toLowerCase().includes(localSearchQuery.toLowerCase().trim());
  ...
  return (
    ...
    >
      ...
      <div className="w-full relative group">
        <Search className="w-5 h-5 text-text-muted group-focus-within:text-primary transition-colors absolute left-4 top-1/2 -translate-y-1/2" />
        <input
          type="text"
          value={localSearchQuery}
          onChange={(e) => setLocalSearchQuery(e.target.value)}
          placeholder="Filter by title, creator, domain category, description details..."
          className="w-full text-xs md:text-sm py-3.5 pl-12 pr-12 rounded-2xl bg-[#13131A] border border-white/5 focus:border-primary/50 text-white placeholder-text-muted outline-none transition-all focus:ring-1 focus:ring-primary/20"
        />
        {localSearchQuery && (
          <button 
        ...
        <div className="glass-card flex flex-col items-center justify-center text-center p-16 border border-white/5 max-w-2xl mx-auto mt-12 rounded-3xl space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted mt-1.5 leading-relaxed">
              We couldn't find any public podcasts matching "{localSearchQuery}" under the category "{selectedCategory}". Try modifying your search filters or resetting keywords.
            </p>
          </div>
          <button
            onClick={() => {
