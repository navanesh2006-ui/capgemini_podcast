...
import { Send, Mic, MicOff, Settings, Sparkles, Bot, User, Volume2, VolumeX, ShieldAlert } from "lucide-react";
...
export default function AiHostTwinPage() {
          ...
          if (timestampRegex.test(part)) {
            ...
            return (
              <button
                ...
                className="inline-flex items-center px-2 py-0.5 mx-1 text-[11px] font-bold rounded bg-primary/25 hover:bg-primary/45 border border-primary/40 text-[#A088FF] hover:text-white transition-all cursor-pointer font-mono select-none"
  ...
  const toggleListening = () => {
    ...
    try {
      baseTextRef.current = inputText; // Save current input text
      ...
      rec.onstart = () => {
        setIsListening(true);
        if (showToast) showToast("Listening... speak now");
      };
      ...
      rec.onerror = (e) => {
        ...
        if (showToast) {
          if (e.error === "not-allowed") {
            showToast("Microphone permission denied. Enable microphone access in browser settings.");
          } else if (e.error === "no-speech") {
            showToast("No speech detected. Please speak clearly.");
          } else if (e.error === "audio-capture") {
            showToast("Microphone hardware error. Check connection.");
          } else {
      ...
      setIsListening(false);
      if (showToast) showToast("Failed to start voice input.");
    }
  };
        ...
        <div className="p-4 border-b border-white/5 flex items-center justify-between shrink-0 bg-background/20">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={twinVoiceMode ? "Mute Digital Twin voice response" : "Unmute Digital Twin voice response"}
            >
              {twinVoiceMode ? <Volume2 className="w-3.5 h-3.5" /> : <VolumeX className="w-3.5 h-3.5" />}
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-white/5 shrink-0 bg-background/20 flex gap-3 items-center">
          ...
          <button
            ...
            className={`p-3.5 rounded-xl border transition-all cursor-pointer flex items-center justify-center shrink-0 ${
              isListening
                ? "bg-red-500/20 border-red-500 text-red-500 animate-pulse shadow-[0_0_12px_rgba(239,68,68,0.3)]"
          ...
          <button
            type="submit"
            className="p-3.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-white hover:opacity-95 active:scale-95 transition-all flex items-center justify-center cursor-pointer shadow-neon-purple shrink-0"
          >
