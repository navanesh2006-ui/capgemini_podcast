...
export default function EmotionTimelinePage() {
  ...
  // Retrieve interpolated emotions at the current time for dots
  const currentEmotions = getInterpolatedEmotion(activeEpisode.id, currentTime);

  // Retrieve interpolated emotions at the hovered time for tooltip
  const hoveredEmotions = hoveredTime !== null ? getInterpolatedEmotion(activeEpisode.id, hoveredTime) : null;

  // Position the floating detail tooltip (constrain between 5% and 80%)
  const hoverPercent = activeEpisode.durationSeconds > 0 && hoveredTime !== null
    ? (hoveredTime / activeEpisode.durationSeconds) * 100 
    : 0;
  const tooltipLeft = Math.min(80, Math.max(5, hoverPercent));
        ...
        {hoveredEmotions && (
          ...
          >
            <p className="font-bold text-white text-[11px] border-b border-white/5 pb-1">
              Seek position: {formatTime(hoveredTime)}
            </p>
            <p className="text-[10px] text-text-muted italic leading-relaxed">
              "{hoveredEmotions.quote}"
            </p>
            <div className="space-y-1.5 border-t border-white/5 pt-2 text-[10px]">
              <div className="flex justify-between items-center">
                ...
                <span className="text-white font-bold">{hoveredEmotions.excitement}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#3B82F6] font-semibold flex items-center gap-1">⚡ Curiosity:</span>
                <span className="text-white font-bold">{hoveredEmotions.curiosity}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#7B5EFF] font-semibold flex items-center gap-1">😮 Surprise:</span>
                <span className="text-white font-bold">{hoveredEmotions.surprise}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#00F5D4] font-semibold flex items-center gap-1">🧠 Deep Thinking:</span>
                <span className="text-white font-bold">{hoveredEmotions.deepThinking}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#00E5A0] font-semibold flex items-center gap-1">😊 Joy:</span>
                <span className="text-white font-bold">{hoveredEmotions.joy}%</span>
              </div>
            </div>
        ...
        <div className="w-full h-80 relative select-none">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              {/* Dashed vertical cursor representing hovered position */}
              {hoveredTime !== null && (
                <ReferenceLine 
                  x={hoveredTime} 
