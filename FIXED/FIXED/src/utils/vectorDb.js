...
export class VectorStore {
  ...
  async generateGeminiEmbedding(text) {
    ...
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-2:embedContent?key=${this.geminiApiKey}`,
      {
        method: "POST",
  ...
  async addEpisodeTranscript(episodeId, fullTranscript) {
    ...
    if (this.provider === "gemini") {
      ...
      try {
        ...
        console.warn("Gemini API embedding test failed. Falling back to local-tfidf:", err);
  ...
  async search(queryText, options = {}) {
    ...
    // Find non-zero indices of the query vector to speed up sparse calculation
    ...
    const results = candidates.map(item => {
      ...
      if (item.vector && item.vector.length > 0 && queryVector && queryVector.length > 0) {
        ...
        const len = itemVector.length;
        
        // Fast sparse dot product
        for (let i = 0; i < nonZeroIndices.length; i++) {
          const idx = nonZeroIndices[i];
          if (idx < len) {
            dotProduct += itemVector[idx] * queryVector[idx];
          }
        }
        score = dotProduct;
      } else {
