...
export default function Navbar() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, text: "AI Host Twin model update completed.", time: "2h ago", unread: true, targetPage: "ai-host-twin" },
    { id: 2, text: "Viral clips generated for Episode 1.", time: "5h ago", unread: true, targetPage: "viral-clips" },
    { id: 3, text: "Weekly Audience Insights report ready.", time: "1d ago", unread: false, targetPage: "audience-insights" }
  ]);
  ...
  const handleNotificationClick = (id, targetPage) => {
      ...
      <div className="flex items-center gap-3 flex-1 max-w-[200px] sm:max-w-md">
        ...
        <div className="w-full relative group">
          ...
          <input
            ...
            placeholder="Search your uploaded podcasts..."
            ...
            <div className="absolute right-0 mt-3 w-80 rounded-2xl bg-[#13131A] border border-white/5 p-4 shadow-2xl z-50">
              ...
              <div className="space-y-2">
                {notifications.map((notif) => (
                  <button
                    key={notif.id}
                    onClick={() => handleNotificationClick(notif.id, notif.targetPage)}
                    className="w-full text-left p-2.5 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 flex flex-col gap-1 transition-all cursor-pointer outline-none"
                  >
                    <div className="flex justify-between items-start w-full">
                      <p className="text-xs text-slate-200 leading-normal pr-2">{notif.text}</p>
