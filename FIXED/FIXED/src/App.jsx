...
function AppContent() {
  const { activePage, setActivePage, selectedReelClip, setSelectedReelClip } = usePodcast();
  ...
  return (
    <div className="min-h-screen bg-background text-white font-sans flex overflow-hidden">
      ...
      {createPortal(<UpgradeModal />, document.body)}
      {createPortal(<ApiConfigModal />, document.body)}
      {selectedReelClip && createPortal(
        <ReelModal 
          clip={selectedReelClip} 
          onClose={() => setSelectedReelClip(null)} 
        />,
        document.body
      )}
      <Toast />
